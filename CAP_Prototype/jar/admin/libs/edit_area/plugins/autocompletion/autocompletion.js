...
var EditArea_autocompletion= {
	...
	,onload: function(){ 
        ...
        //create autocompletion container
		this.container	= document.createElement('div');
		this.container.id	= "auto_completion_area";
        //insert it into editor
						...
						for( var prefix in datas["KEYWORDS"] )
						{
							...
							for( var j=0; j<datas["KEYWORDS"][prefix].length; j++ )
							{
								tmp["keywords"][prefix]['datas'][j]= {
                                    ...
                                    returntype: datas["KEYWORDS"][prefix][j][3] ? datas["KEYWORDS"][prefix][j][3] : ''
								};
		...
		if( editArea.is_editable )
		{
			...
			else
			{
				...
				for(var i=0; i<results.length; i++)
				{
                    ...
                    if(results[i][1]['returntype'].length>0) {
                        line+='<span class="prefix">'+ results[i][1]['returntype'] +'</span>';
                    } else if(results[i][0]['prefix_name'].length>0) {
